...
func (h *historicalEnqueuer) buildSeries(ctx context.Context, bctx *buildSeriesContext) (hardErr, softErr error) {
	...
	nearestCommit, err := findNearestCommit(ctx, bctx.repo.Name, "HEAD", frameMidpoint)
	if err != nil {
		if gitserver.IsRevisionNotFound(err) || vcs.IsRepoNotExist(err) {
...
//
// Can theoretically return nil, nil if no commits at all are found. Can return a commit very far
// away if no nearby one exists.
//
// TODO(slimsag): move to internal/vcs/git
func findNearestCommit(ctx context.Context, repoName api.RepoName, revSpec string, target time.Time) (*git.Commit, error) {
	if revSpec == "" {
		revSpec = "HEAD"
	}
	// Resolve e.g. the branch we're looking at.
	branchCommit, err := git.ResolveRevision(ctx, repoName, revSpec, git.ResolveRevisionOptions{NoEnsureRevision: true})
	if err != nil {
		return nil, err
	}

	// Find the next closest commit on or after our target time.
	commitsAfter, err := git.Commits(ctx, repoName, git.CommitsOptions{
		N:     1,
